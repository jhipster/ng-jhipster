...
describe('Parse links service test', function () {
    describe('Parse Links Service Test', function () {
        beforeEach(function () {
        ...
        it('should throw an error when passed an empty string', inject([ParseLinks], function (service) {
            expect(function () { service.parse(''); }).toThrow(new Error('input must not be of zero length'));
        }));
        it('should throw an error when passed without comma', inject([ParseLinks], function (service) {
            expect(function () { service.parse('test'); }).toThrow(new Error('section could not be split on ";"'));
        }));
        it('should throw an error when passed without semicolon', inject([ParseLinks], function (service) {
            expect(function () { service.parse('test,test2'); }).toThrow(new Error('section could not be split on ";"'));
        }));
        it('should return links when headers are passed', inject([ParseLinks], function (service) {
